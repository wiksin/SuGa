...
namespace Nop.Plugin.Payments.WxPay.app_code
{
    public class Refund
    {
        ...
        public Refund(ConfigurationModel modoel)
        {
            WxPayConfig = modoel;
        }
        /***
        ...
        */
        public  string Run(string transaction_id, string out_trade_no, string total_fee, string refund_fee)
        {
            ...
            data.SetValue("out_refund_no",new WxPayApi(WxPayConfig).GenerateOutTradeNo());//随机生成商户退款单号
            data.SetValue("op_user_id", WxPayConfig.MCHID);//操作员，默认为商户号

            WxPayData result = new WxPayApi(WxPayConfig).Refund(data);//提交退款申请给API，接收返回数据
